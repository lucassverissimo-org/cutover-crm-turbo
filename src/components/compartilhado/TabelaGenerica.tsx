...
const TabelaGenerica = forwardRef<TabelaGenericaRef, TabelaGenericaProps>(
  ({ tabela, colunas, onEditar, onDeletar }, ref) => {
    const [registros, setRegistros] = useState<any[]>([])

    const buscarDados = async () => {
      if (supabase == null) return
      const { data } = await supabase.from(tabela).select('*').order('created_at')
      setRegistros(data || [])
    }
              ...
              {registros.map((registro, i) => (
                <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="p-3 border-b space-x-2">
                    <button onClick={() => onEditar?.(registro)} title="Editar">✏️</button>
                    <button onClick={() => onDeletar?.(registro)} title="Deletar">🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
